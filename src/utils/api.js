...
export function fetchCategoriesJson() {
  ...
  return fetch(url, { headers: { Authorization: "ak" } }).then(c => c.json());
}

export function fetchPostsJson() {
  let url = "http://localhost:3001/posts";
  return fetch(url, { headers: { Authorization: "a" } }).then(posts =>
    posts.json()
  );
}
...
export function fetchCommentsJson(pid) {
  ...
  return fetch(url, { headers: { Authorization: "a" } }).then(comments =>
    comments.json()
  );
}
